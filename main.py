...
def train_one_time(args, state_logger):
    ...
    sampler_train = IncompleteDatasetSampler(
        dataset_train,
        seed=args.seed
    )
    ...
    if args.missing_rate >= 0.0:
        ...
        sampler_train_all = IncompleteDatasetSampler(
            dataset_all,
            seed=args.seed
        )
    ...
    if args.train_id == 0:
        ...
        state_logger.write(
            'Train parameters: {}'.format(args).replace(', ', ',\n')
        )
        state_logger.write(model.__repr__())
        state_logger.write(optimizer.__repr__())

        print(
            'Data loaded: there are {:} samples.'.format(
                len(dataset_train)
            )
        )

    state_logger.write(
        '\n>> Start training {}-th initial, seed: {},'.format(
            args.train_id,
            args.seed
        )
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        args.print_this_epoch = (
            epoch % args.print_freq == 0
            or epoch + 1 == args.epochs
        )

        # 控制实际评估频率
        # epoch 100 之后每个 epoch 都评估，以免错过最佳结果
        args.evaluate_this_epoch = (
            epoch >= 100
            or args.print_this_epoch
        )
        ...
        # 只按照打印频率输出评估指标
        if args.print_this_epoch and train_state is not None:
            state_logger.write(
                'Epoch {} K-means: '
                'NMI = {:.4f} ARI = {:.4f} '
                'F = {:.4f} ACC = {:.4f}'.format(
        ...
        # epoch 100 之后，每个实际评估的 epoch 都参与最佳结果比较
        if (
            args.evaluate_this_epoch
            and train_state is not None
            and epoch >= 100
            and train_state['nmi'] >= best_result['nmi']
            and train_state['ari'] >= best_result['ari']
            and train_state['f'] >= best_result['f']
            and train_state['acc'] >= best_result['acc']
        ):
...
def main(args):
    ...
    for t in range(args.train_time):
        ...
        train_state, best_result = train_one_time(
            args,
            state_logger
        )
